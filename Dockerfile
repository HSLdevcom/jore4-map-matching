...
# setup for "dev" profile
FROM base as devbase
COPY ./spring-boot/profiles/dev/config.properties /build/profiles/dev/config.properties
RUN touch /build/profiles/dev/config.$(whoami).properties
