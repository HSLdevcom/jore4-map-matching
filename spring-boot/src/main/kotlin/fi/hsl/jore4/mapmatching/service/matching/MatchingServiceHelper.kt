        ...
        return startPointCandidates
            ...
            .sortedWith { terminiCandidate1, terminiCandidate2 ->
                ...
                if (stopCount1 > stopCount2)
                    ...
                    // delegate to closest distance comparison

                    fun getClosestDistance(terminiCandidate: Pair<TerminusPointCandidate, TerminusPointCandidate>): Double =
                        min(terminiCandidate.first.closestDistance, terminiCandidate.second.closestDistance)

                    fun getFurthestDistance(terminiCandidate: Pair<TerminusPointCandidate, TerminusPointCandidate>): Double =
                        max(terminiCandidate.first.closestDistance, terminiCandidate.second.closestDistance)

                    // sort terminus point candidate pairs by closest distances

                    val closestDistance1: Double = getClosestDistance(terminiCandidate1)
                    val closestDistance2: Double = getClosestDistance(terminiCandidate2)

                    if (closestDistance1 < closestDistance2)
                        -1
                    else if (closestDistance1 > closestDistance2)
                        1
                    else
            ...
            .map { (startPointCandidate, endPointCandidate) ->
                ...
                val routePoints: MutableList<PgRoutingPoint> = ArrayList()
                routePoints.add(startPointCandidate.targetPoint)
                routePoints.addAll(viaRoutePoints)
                routePoints.add(endPointCandidate.targetPoint)
