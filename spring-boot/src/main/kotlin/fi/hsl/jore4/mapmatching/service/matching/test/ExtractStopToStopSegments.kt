...
object ExtractStopToStopSegments {
    ...
    private data class SegmentInfo(val routePoints: List<RoutePoint>,
    ...
    private data class RoutePointLocations(val measuredLocation: Point<G2D>,
        ...
        fun hasSharedPositionWith(other: RoutePointLocations): Boolean {
            return hasPosition(other.measuredLocation.position)
                || other.projectedLocation?.run { hasPosition(position) } == true
        }
    }

    private fun RoutePoint.toLocations() = when (this) {
    ...
    fun extractStopToStopSegments(routes: List<PublicTransportRoute>): StopToStopSegmentation {
        ...
        val stopToStopSegments = segmentMap
            .toSortedMap()
            .entries
            .flatMap { (stopToStopSegmentId, segmentMap) ->
                ...
                segmentMap.entries
                    ...
                    .map { (index, segmentEntry) ->
                        ...
                        val stopToStopRouteId = if (index == 0)
                            "$stopToStopSegmentId"
                        else
    ...
    private fun splitToStopToStopSegments(route: PublicTransportRoute): List<PublicTransportRoute> {
        ...
        val terminusOfRoutePointSegments: List<Pair<RouteStopPoint, RouteStopPoint>> = routePointSegments
            .map { routePoints ->
        ...
        val geometrySegments: List<LineString<G2D>> = splitRouteGeometry(route.routeId,
                                                                         route.routeGeometry,
        ...
        fun getStopId(routePoint: RoutePoint): String = when (routePoint) {
            is RouteStopPoint -> {
        ...
        return geometrySegments
            ...
            .map { (lineString, routePoints) ->
                ...
                PublicTransportRoute("${getStopId(routePoints.first())}-${getStopId(routePoints.last())}",
                                     lineString,
                                     routePoints)
            }
    }
    ...
    private fun trimRoutePointStart(route: PublicTransportRoute): List<RoutePoint> {
        ...
        return if (route.routePoints.first().toLocations().hasPosition(route.routeGeometry.startPosition))
            route.routePoints
        else {
            LOGGER.info {
    ...
    private fun extractRoutePointIndexRangesForStopToStopSegments(routePoints: List<RoutePoint>): List<Pair<Int, Int>> {
        ...
        val routeStopPointIndices: List<Int> = routePoints.mapIndexedNotNull { index, routePoint ->
    ...
    private fun splitRouteGeometry(routeId: String,
                                   routeGeometry: LineString<G2D>,
                                   stopPointRanges: List<Pair<RouteStopPoint, RouteStopPoint>>,
                                   printDebug: Boolean): List<LineString<G2D>> {
        ...
        val (_, splitLineStrings: List<LineString<G2D>>) = stopPointRanges
            .fold(trimmedLine to emptyList<LineString<G2D>>()) { (remainingLineString, substrings), (_, endStopPoint) ->
    ...
    private fun takeFromStart(lineString: LineString<G2D>, toLocation: RoutePointLocations): LineString<G2D> {
    ...
    private fun dropFromStart(lineString: LineString<G2D>, untilLocation: RoutePointLocations): LineString<G2D> {
        ...
        val numPositionsToDrop: Int = when {
