...
@InternalService
class MatchRouteViaPointsOnLinksServiceImpl @Autowired constructor(
    ...
    override fun findMatchForPublicTransportRoute(sourceRouteGeometry: LineString<G2D>,
        ...
        val targetRoutePointSequenceCandidates: List<List<PgRoutingPoint>> =
            resolveRoutePointSequenceCandidates(sourceRoutePoints,
                                                vehicleType,
                                                terminusLinkSelectionInput,
                                                matchingParameters)

        return findFirstMatchingRouteOrNull(sourceRouteGeometry,
                                            vehicleType,
    ...
    internal fun resolveRoutePointSequenceCandidates(sourceRoutePoints: List<RoutePoint>,
                                                     vehicleType: VehicleType,
        ...
        ) = resolveTerminusPointCandidatesAndStopPoints(sourceRoutePoints,
                                                        terminusLinkSelectionInput,
        ...
        val fromRoutePointIndexToRoadJunctionNode: Map<Int, NodeProximity?> = matchingParams.roadJunctionMatching
            ?.let { params ->
                roadJunctionMatcher.findInfrastructureNodesMatchingRoadJunctions(sourceRoutePoints, vehicleType, params)
            }
            ?: emptyMap()

        val targetViaRoutePoints: List<PgRoutingPoint> = sourceRoutePoints
            .withIndex()
            .drop(1)
            .dropLast(1)
        ...
        return MatchingServiceHelper.getSortedRoutePointSequenceCandidates(targetStartPointCandidates,
                                                                           targetEndPointCandidates,
                                                                           targetViaRoutePoints)
    }

    internal fun resolveTerminusPointCandidatesAndStopPoints(sourceRoutePoints: List<RoutePoint>,
                                                             terminusLinkSelectionInput: TerminusLinkSelectionInput,
        ...
        val (targetStartPointCandidates: List<TerminusPointCandidate>,
            targetEndPointCandidates: List<TerminusPointCandidate>) =
            MatchingServiceHelper.createPairwiseCandidatesForRouteTerminusPoints(terminusLinkSelectionInput,
        ...
        return TerminusPointCandidatesAndStopPoints(targetStartPointCandidates,
                                                    targetEndPointCandidates,
                                                    fromRoutePointIndexToTargetStopPoint)
    }

    internal fun findFirstMatchingRouteOrNull(sourceRouteGeometry: LineString<G2D>,
                                              vehicleType: VehicleType,
        ...
        return targetRoutePointSequenceCandidates.firstNotNullOfOrNull { targetRoutePoints ->
            ...
            val bufferAreaRestriction = BufferAreaRestriction.from(sourceRouteGeometry,
