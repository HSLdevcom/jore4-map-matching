...
data class SnappedLinkState(val infrastructureLinkId: InfrastructureLinkId,
    ...
    fun withSnappedToTerminusNode(thresholdDistanceOfSnappingToLinkEndpointInMeters: Double): SnappedLinkState {
        return createWithLocationSnappedToEndpointIfClose(infrastructureLinkId,
                                                          closestDistance,
    ...
    companion object {
        ...
        fun createWithLocationSnappedToEndpointIfClose(infrastructureLinkId: InfrastructureLinkId,
                                                       closestDistance: Double,
                                                       closestPointFractionalMeasure: Double,
                                                       trafficFlowDirectionType: TrafficFlowDirectionType,
                                                       linkLength: Double,
                                                       startNodeId: InfrastructureNodeId,
                                                       endNodeId: InfrastructureNodeId,
                                                       snapToLinkEndpointDistanceInMeters: Double)
            : SnappedLinkState {

            val distanceFromStartOfLink: Double = closestPointFractionalMeasure * linkLength

            val closestPointFractionalMeasureSnappedToEndpoint: Double =
                if (distanceFromStartOfLink.isWithinTolerance(0.0, snapToLinkEndpointDistanceInMeters))
                    0.0
                else if (distanceFromStartOfLink.isWithinTolerance(linkLength, snapToLinkEndpointDistanceInMeters))
                    1.0
                else
                    closestPointFractionalMeasure
            ...
            return SnappedLinkState(infrastructureLinkId,
                                    closestDistance,
                                    closestPointFractionalMeasureSnappedToEndpoint,
                                    trafficFlowDirectionType,
                                    linkLength,
                                    startNodeId,
                                    endNodeId)
