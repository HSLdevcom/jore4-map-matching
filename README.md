...
| `maxStopLocationDeviation`     | The maximum distance within which two locations given for a public transport stop are allowed to be away from each other, in order to include the stop in the set of route points that are matched with infrastructure links. The first location is the one hosted in this map-matching service (mostly originating from Digiroad) and the second one is the location defined within the client system (invoking this map-matching service). If the distance between these two type of locations exceeds `maxStopLocationDeviation`, then the affected stops are discarded from the set of route points that are matched with infrastructure links. Defaults to 80 meters. |
